...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, OrderInfo> implements OrderService {
    ...
    public Long saveOrder(String scheduleId, Long patientId) {
        ...
        if (null == patient) throw new YyghException(ResultCodeEnum.PARAM_ERROR);
        //排班
        ScheduleOrderVo scheduleOrderVo = hospitalFeignClient.getScheduleOrderVo(scheduleId);
        if (null == scheduleOrderVo) throw new YyghException(ResultCodeEnum.PARAM_ERROR);
        //当前时间是否可以预约
        ...
        if (null == signInfoVo) throw new YyghException(ResultCodeEnum.PARAM_ERROR);
        if (scheduleOrderVo.getAvailableNumber() <= 0) throw new YyghException(ResultCodeEnum.NUMBER_NO);
